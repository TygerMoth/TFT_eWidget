...
{
  int16_t _yd_min = 5; // added int16_t vartiable _yd_min = 5 (to allow minimum button heights of 12 px)
  _xd        = x_delta;
...
void ButtonWidget::drawButton(bool inverted, String long_name) {
  ...
  if (_tft->textfont == 255) {
    ...
    if (long_name == "")
      ...
      _tft->drawString(long_name, _x1 + (_w/2) + _xd, _y1 + (_h/2) + _yd); // after adding max(..., 5) to _yd above
